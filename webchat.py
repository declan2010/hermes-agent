...
def _load_hermes_config():
    ...
    try:
        ...
        return default_model, fallback_models, base_url, provider
    ...
    with sessions_lock:
        if session_id not in sessions:
            ...
            if env_model:
                ...
                default_model, fallback_models, base_url, provider = _load_hermes_config()
                model = default_model
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .message .content thead { border-bottom: 2px solid var(--accent); }
        .message .content th {
            background: var(--bg-tertiary);
            color: var(--accent-light);
            font-weight: 600;
        ...
        [data-theme="light"] .message .content tr:hover td {
...
<body>
    ...
    <div class="status-bar">
        <div class="status-left">
            <div class="status-item">
                ...
                <span id="modelName">{{ default_model }}</span>
            </div>
    ...
    <script>
        ...
        function updateTime() {
            ...
            sessionTime.textContent = formatElapsed(elapsed);
        }
        ...
        function addMessage(role, content, timestamp) {
            ...
            // Add content - render Markdown for assistant, plain text for user
...
@app.route('/')
def index():
    model_name, _, _, _ = _load_hermes_config()
    return render_template_string(HTML_TEMPLATE, default_model=model_name.split(':')[0])
...
def chat():
    ...
    try:
        ...
        fallback_models = session.get('fallback_models', [])
        if fallback_models:
...
def get_session_history(session_id_to_load):
    ...
    try:
        from hermes_state import SessionDB
        db = SessionDB()
        messages = db.get_messages_as_conversation(session_id_to_load, include_timestamp=True)
        result = []
        for msg in messages:
